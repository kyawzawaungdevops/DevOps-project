    ...
    stages {
        stage('Repo Scan using  Sonarcloud'){
         steps {
          ...
          sh "mvn verify org.sonarsource.scanner.maven:sonar-maven-plugin:sonar -Dsonar.projectKey=devops-projectslabs_kyaw -DskipTests=true -Dmaven.multiModuleProjectDirectory=true"
         }
      }
     }
